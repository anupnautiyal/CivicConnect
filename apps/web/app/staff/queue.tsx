...
export async function StaffQueue({role,query}:{role:Role;query:Record<string,string|undefined>}) {
 ...
 return <><header><Link className="brand" href={base}><span className="mark">C</span>CivicConnect</Link><span className="pill">{role==="OFFICER"?"Municipal officer":"Administrator"}</span><ProfileMenu name={profile.display_name} role={profile.role}/></header>
