...
export default async function Page({searchParams}:{searchParams:Promise<{page?:string}>}) {
 ...
 return <><header><Link href="/citizen" className="brand"><span className="mark">C</span>CivicConnect</Link><span className="pill">Citizen</span><form action={logout}><button className="secondary">Sign out</button></form></header>
 ...
 <nav className="auth-links" aria-label="Complaint pages">{page>1&&<Link href={"/citizen?page="+(page-1)}>Previous page</Link>}{count!==null&&count!==undefined&&page*10<count&&<Link href={"/citizen?page="+(page+1)}>Next page</Link>}<Link href="/update-password">Change password</Link></nav></main></>;
}
