...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}) {
 ...
 const purpose=new URL(request.url).searchParams.get("purpose")==="RESOLUTION"?"RESOLUTION":"REPORT";
 ...
 const {data:media,error}=await supabase.from("issue_media").select("storage_path").eq("issue_id",id).eq("purpose",purpose).order("created_at",{ascending:false}).limit(1).maybeSingle();
