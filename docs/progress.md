...
## UI refresh and staff signup requests

Added indigo/mint styling, responsive auth split screens with lightweight CSS city artwork, three signup role choices, password visibility control, redesigned landing page, shared dashboard cards and filters, reduced-motion support, profile menu and profile page. Public signup keeps all new users as citizens; staff requests require service-role owner review after independent municipal employment verification and confirmed email. See docs/operations/staff-access.md. Migration 202609150009_staff_access_requests.sql awaits hosted application. All 16 tests and production build passed. Browser control failed to initialize; visual acceptance remains pending. Sprint 5 stays paused. Changes are local and unpushed.
